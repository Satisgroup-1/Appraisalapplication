...
export function getCaseVariants(study: CaseStudy, research: ResearchFinding[]): Record<'simple' | 'advanced', ReportVariant<CaseExhibitPlacement>> {
  ...
  return {
    simple: {
      ...
      opening: { label: study.status === 'In progress' ? 'Representative engagement item' : 'Composite operating item', title: opening.title, paragraphs: opening.paragraphs, centralQuestion: advanced.centralQuestion },
      sections: simpleSections,
