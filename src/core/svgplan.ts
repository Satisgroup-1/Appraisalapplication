...
// Monochrome tints per unit label (brand palette derivatives).
...
export function planToSvg(plan: FloorPlanResult, env: Envelope, opts?: { title?: string }): string {
  ...
  const s: string[] = [
    ...
    `<text x="${PAD}" y="28" font-size="13" letter-spacing="3" fill="#000000">${esc(
      (opts?.title ?? `PROPOSED CONVERSION — FLOOR ${plan.floor}`).toUpperCase(),
    )}</text>`,
    `<line x1="${PAD}" y1="38" x2="${width - PAD}" y2="38" stroke="#000000" stroke-width="0.75"/>`,
    `<text x="${PAD}" y="52" font-size="10" fill="#6b6f71">${esc(
  ...
  s.push(`<polygon points="${pts}" fill="#fafafa" stroke="#000000" stroke-width="2.5"/>`);

  // corridor
  if (plan.corridor) {
    const c = plan.corridor;
    s.push(
      `<rect x="${X(ox)}" y="${Y(c.y0)}" width="${px(W)}" height="${px(c.y1 - c.y0)}" fill="#f2f3f3" stroke="#9ea3a5" stroke-dasharray="4 3" stroke-width="0.75"/>`,
      `<text x="${X(ox) + 6}" y="${Y(c.y0) + px(c.y1 - c.y0) / 2 + 3}" font-size="8" letter-spacing="2" fill="#9ea3a5">CORRIDOR</text>`,
    );
  }
  ...
  for (const core of env.cores) {
    ...
    s.push(
      `<polygon points="${cpts}" fill="#ced1d2" stroke="#000000" stroke-width="0.75"/>`,
      `<text x="${X(cx)}" y="${Y(cy) + 3}" font-size="8" letter-spacing="2" text-anchor="middle" fill="#000000">${esc(
  ...
  for (const u of plan.units) {
    const fill = TYPE_FILL[u.label] ?? '#e9eaeb';
    s.push(
      `<rect x="${X(u.x0)}" y="${Y(u.y0)}" width="${px(u.x1 - u.x0)}" height="${px(u.y1 - u.y0)}" fill="${fill}" stroke="#000000" stroke-width="1.75"/>`,
    );
    for (const r of u.rooms) {
      ...
      s.push(
        `<rect x="${X(r.x)}" y="${Y(ry)}" width="${px(r.w)}" height="${px(r.d)}" fill="none" stroke="#9ea3a5" stroke-width="0.6"/>`,
        `<text x="${X(r.x) + 3}" y="${Y(ry) + 11}" font-size="7.5" fill="#4d5254">${esc(r.name)} ${r.area}m²</text>`,
      );
    }
    const midx = (u.x0 + u.x1) / 2;
    const midy = (u.y0 + u.y1) / 2;
    s.push(
      `<text x="${X(midx)}" y="${Y(midy) + 4}" font-size="11" font-weight="600" text-anchor="middle" fill="#000000">${u.no}: ${esc(
        u.label,
      )} ${u.giaSqm}m²</text>`,
    );
  }
  ...
  for (const w of env.windows) {
    ...
    s.push(
      `<line x1="${X(w.x - 0.5)}" y1="${Y(wy)}" x2="${X(w.x + 0.5)}" y2="${Y(wy)}" stroke="#000000" stroke-width="5"/>`,
    );
  }
